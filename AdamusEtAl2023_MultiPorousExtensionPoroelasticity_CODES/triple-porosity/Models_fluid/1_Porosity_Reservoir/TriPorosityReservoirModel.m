classdef TriPorosityReservoirModel < PhysicalModel
   
    % DESCRIPTION:
    %   Extension of `PhysicalModel` class to accomodate model-specific
    %   features such as fluid and rock as well as used phases and variables.
...
properties
    ...
    maximumPressure  % Maximum pressure allowed in the model
    minimumPressure  % Minimum pressure allowed in the model
...
methods
    % --------------------------------------------------------------------%
    function model = TriPorosityReservoirModel(G, varargin)
        ...
        %% Triple porosity: rock and fluid fracture/matrix/matrix2
        model.rock_matrix    = rock_all{2};
        model.fluid_matrix   = fluid_all{2};
        model.rock_matrix2   = rock_all{3};
        model.fluid_matrix2  = fluid_all{3};
        
        %% Triple porosity: adding 'dummy' transfer object
        ...
        [model, unparsed] = merge_options(model, varargin{:}); 
        ...
        if doSetup
            ...
            %% Triple porosity: micropore pore volume
            operators_matrix2 = setupOperatorsTPFA(G, model.rock_matrix2, 'deck', model.inputdata);
            model.operators.pv_matrix2 = operators_matrix2.pv;
        end
    end
    ...
    function vars = getSaturationVarNamesMatrix2(model)
        vars = {'swm2', 'som2', 'sgm2'};
        ph = model.getActivePhases();
        vars = vars(ph);
    end
    ...
    function [state, report] = updateState(model, state, problem, dx, drivingForces)
        ...
        [restVars, satVars, satMatrixVars, satMatrix2Vars, wellVars] = model.splitPrimaryVariables(problem.primaryVariables);
        ...
        % Update saturations in one go
        state  = model.updateSaturationsMatrix2(state, dx, problem, satMatrix2Vars);
    ...
    function [values, tolerances, names] = getConvergenceValues(model, problem, varargin)
        ...
        if model.useCNVConvergence
            ...
            % Use strict tolerances on the residual 
    ...
    function [fn, index] = getVariableField(model, name, varargin)
        ...
        %% Triple porosity: variable names
        switch(lower(name))
            case {'t', 'temperature'}
                fn = 'T';
                index = ':';
            ...
            case {'pressure_matrix', 'pm'}
                index = ':';
                fn = 'pressure_matrix';
            case {'swm2', 'water_matrix2'}
                index = model.satVarIndexMatrix('swm2');
                fn = 'sm2';
            case {'som2', 'oil_matrix2'}
                index = model.satVarIndexMatrix('som2');
                fn = 'sm2';
            case {'sgm2', 'gas_matrix2'}
                index = model.satVarIndexMatrix('sgm2');
                fn = 'sm2';
            case {'sm2', 'satm2', 'saturation_matrix2'}
                index = ':';
                fn = 'sm2';
            case {'pressure_matrix2', 'pm2'}
                index = ':';
                fn = 'pressure_matrix2';
            case 'wellsol'
                % Use colon to get all variables, since the wellsol may be empty
                index = ':';
                fn = 'wellSol';
            otherwise
    ...
    % --------------------------------------------------------------------%
    function [restVars, satVars, satMatrixVars, satMatrix2Vars, wellVars] = splitPrimaryVariables(model, vars)
        ...
        isSatMatrix2 = cellfun(@(x) any(strcmpi(model.getSaturationVarNamesMatrix2, x)), vars);

        wellVars = vars(isWells);
        satVars  = vars(isSat);
        satMatrixVars = vars(isSatMatrix);
        satMatrix2Vars = vars(isSatMatrix2);

        restVars = vars(~isSat & ~isWells & ~isSatMatrix & ~isSatMatrix2);
    end
    ...
    function [isActive, phInd] = getActivePhases(model)
        ...
        %              indicating if that phase is present. Ordering of water, oil and then gas.
        
        %   phInd    - Indices of the phases present. For instance, if
        %              water and gas are the only ones present, `phInd = [1, 3]`
    ...
    function state = updateSaturationsMatrix(model, state, dx, problem, satMatrixVars)
        ...
        state_init = state;
        % Solution variables should be saturations directly, find the missing link
     ...
     % --------------------------------------------------------------------%
    function state = updateSaturationsMatrix2(model, state, dx, problem, satMatrix2Vars)
        % Update of phase-saturations
        ...
        %   Update saturations (likely state.s) under the constraint that
        %   the sum of volume fractions is always equal to 1. This
        %   assumes that we have solved for n - 1 phases when n phases are present.
        ...
        if isempty(satMatrix2Vars)
            % No saturations passed, nothing to do here.
            return
        end
        state_init = state;
        % Solution variables should be saturations directly, find the missing link
        saturations = lower(model.getSaturationVarNamesMatrix2);
        fillsat = setdiff(saturations, lower(satMatrix2Vars));
        ...
        % We update all saturations simultanously, since this does not bias the
        % increment towards one phase in particular.
        state = model.updateStateFromIncrement(state, ds, problem, 'sm2', inf, model.dsMaxAbs);
        if n_fill == 1
            % Ensure that values are within zero->one interval, and
            % re-normalize if any values were capped
            bad = any((state.sm2 > 1) | (state.sm2 < 0), 2);
            if any(bad)
                state.sm2(bad, :) = min(state.sm2(bad, :), 1);
                state.sm2(bad, :) = max(state.sm2(bad, :), 0);
                state.sm2(bad, :) = bsxfun(@rdivide, state.sm2(bad, :), sum(state.sm2(bad, :), 2));
            end
        else
            % Ensure positive values, we assume that sum of saturations is
            % handled via constraint equation
            state.sm2 = max(state.sm2, 0);
        end
    end
    ...
    function state = storeBoundaryFluxes(model, state, qW, qO, qG, forces)
        ...
        % to the neighbor structure in the grid itself.
    ...
    function index = satVarIndexMatrix2(model, name)
        % Find the index of a saturation variable by name
        ...
        index = find(strcmpi(model.getSaturationVarNamesMatrix2, name));
    end
...
% --------------------------------------------------------------------%
    function scaling = getScalingFactorsCPR(model, problem, names, solver) 
        ...
        %             each equation, or a vector of equal length to that equation.
...
% --------------------------------------------------------------------%
    ...
    function [eqs, state, src] = addBoundaryConditionsAndSources(model, eqs, names, types, state, ...
        ...
        %                sub-component is passed onto `addComponentContributions`.
...
methods (Static)
    ...
    function ds = adjustStepFromSatBounds(s, ds)
        ...
        % the same length, in a manner that avoids saturation violations.
        tmp = s + ds;
